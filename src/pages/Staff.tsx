...
const roleLabels: Record<UserRole, string> = {
  ...
  LABORATOIRE: 'Secrétaire Labo',
  PHARMACIE: 'Pharmacien',
  PATIENT: 'Patient',
};

const specialitiesByRole: Record<string, string[]> = {
  MEDECIN: [
    'Cardiologie', 'Neurologie', 'Pédiatrie', 'Gynécologie',
    'Médecine Générale', 'Dermatologie', 'Oncologie',
    'Chirurgie Orthopédique', 'Psychiatrie', 'Ophtalmologie'
  ],
  INFIRMIER: [
    'Soins Intensifs', 'Urgences', 'Bloc Opératoire',
    'Pédiatrie CARE', 'Gérontologie', 'Santé Mentale'
  ],
  TECHNICIEN: [
    'Radiologie', 'Scanner/IRM', 'Maintenance Biomédicale'
  ],
  LABORATOIRE: [
    'Hématologie', 'Biochimie', 'Microbiologie', 'Immunologie'
  ],
  PHARMACIE: [
    'Pharmacie Clinique', 'Gestion des Stocks', 'Dispensation', 'Pharmacie Hospitalière'
  ],
  SECRETAIRE: [
    'Accueil & Admission', 'Facturation', 'Archives Médicales'
  ],
  ADMIN: [
    'Direction Générale', 'Ressources Humaines', 'Systèmes d\'Information', 'Comptabilité'
  ],
  AIDE_SOIGNANT: [
    'Hygiène & Confort', 'Assistance au CARE'
  ],
  PATIENT: ['N/A']
};
...
const Staff = () => {
  ...
  const addMutation = useMutation({
    ...
    onError: (error: any) => {
      ...
      // Handle "Un staff avec cet email existe déjà" specifically if needed, but the generic message handler usually covers it
  ...
  const filteredStaff = staff.filter(member => {
    ...
    return `${prenom} ${nom}`.toLowerCase().includes(search) ||
              ...
              <form onSubmit={handleAddStaff}>
                ...
                <div className="grid gap-4 py-4">
                  ...
                  <div className="grid grid-cols-4 items-center gap-4">
                    ...
                    <Select value={newStaff.type} onValueChange={(v: UserRole) => {
                      setNewStaff({
                        ...newStaff,
                        type: v,
                        specialite: specialitiesByRole[v]?.[0] || ''
                      });
                    }}>
                      ...
                      <SelectContent>
                        {Object.entries(roleLabels).map(([val, label]) => (
                          val !== 'PATIENT' && val !== 'ADMIN' && <SelectItem key={val} value={val}>{label}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                  </div>
                  <div className="grid grid-cols-4 items-center gap-4">
                    ...
                    <Select
                      ...
                      onValueChange={(v) => setNewStaff({ ...newStaff, specialite: v })}
                    >
                      <SelectTrigger className="col-span-3">
                        <SelectValue placeholder="Sélectionnez une spécialité" />
                      </SelectTrigger>
                      <SelectContent>
                        {specialitiesByRole[newStaff.type || 'MEDECIN']?.map((spec) => (
            ...
            <form onSubmit={handleUpdateStaff}>
              ...
              <div className="grid gap-4 py-4">
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Select value={editingStaff.type} onValueChange={(v: UserRole) => {
                    setEditingStaff({
                      ...editingStaff,
                      type: v,
                      specialite: specialitiesByRole[v]?.[0] || ''
                    ...
                    <SelectContent>
                      {Object.entries(roleLabels).map(([val, label]) => (
                        val !== 'PATIENT' && <SelectItem key={val} value={val}>{label}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setEditingStaff({ ...editingStaff, specialite: v })}
                  >
                    <SelectTrigger className="col-span-3">
                      <SelectValue placeholder="Sélectionnez une spécialité" />
                    </SelectTrigger>
                    <SelectContent>
                      {specialitiesByRole[editingStaff.type || 'MEDECIN']?.map((spec) => (
                        <SelectItem key={spec} value={spec}>{spec}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
